...
///
// -------------------------------------------------------------------------------------------------
DCL-PROC VLD_GetMsg EXPORT;
  DCL-PI VLD_GetMsg LIKE(tpl_sdk4i_vldmsgt_ds.msg);
  ...
  DCL-S lng_id LIKE(tpl_sdk4i_vldmsgt_ds.lng_id) INZ('en');
  DCL-S msg LIKE(tpl_sdk4i_vldmsgt_ds.msg) INZ('An error occurred retrieving a message.');
  ...
  // Main logic.
  // --------------------------------------------------
  // If we were given a language code, use it instead of the default value.
  IF (%PARMS >= %PARMNUM(i_lng_id) AND %ADDR(i_lng_id) <> *NULL);
    lng_id = i_lng_id;
  ENDIF;

  // Get the message.
  s_stmt = 'SELECT msg FROM vldmsgt WHERE id = ? AND lng_id = ?';

  EXEC SQL PREPARE s_GetMsg FROM :s_stmt;
  ...
  EXEC SQL DECLARE c_GetMsg CURSOR FOR s_GetMsg;
  IF (ERR_IsSQLError(s_diagnostics_ds: *OMIT: *OMIT: log_user_info_ds));
    log_is_successful = *OFF;
    log_cause_info_ds.sstate = s_diagnostics_ds.returned_sqlstate;
    log_cause_info_ds.sstmt = s_stmt;
  ...
  EXEC SQL OPEN c_GetMsg USING :i_id, :lng_id;
  IF (ERR_IsSQLError(s_diagnostics_ds: *OMIT: *OMIT: log_user_info_ds));
    log_is_successful = *OFF;
    log_cause_info_ds.sstate = s_diagnostics_ds.returned_sqlstate;
    log_cause_info_ds.sstmt = s_stmt;
...
DCL-PROC VLD_IsValid EXPORT;
  DCL-PI VLD_IsValid IND;
    i_tbl LIKE(tpl_sdk4i_vldrult_ds.tbl) OPTIONS(*TRIM) CONST;
    i_col LIKE(tpl_sdk4i_vldrult_ds.col) OPTIONS(*TRIM) CONST;
    o_vldmsgt_id LIKE(tpl_sdk4i_vldrult_ds.vldmsgt_id);
    i_str LIKE(tpl_sdk4i_sql_statement) OPTIONS(*NOPASS: *NULLIND: *OMIT) CONST;
    i_num LIKE(tpl_sdk4i_vldrult_ds.max_num) OPTIONS(*NOPASS: *NULLIND: *OMIT) CONST;
    i_date LIKE(tpl_sdk4i_vldrult_ds.max_date) OPTIONS(*NOPASS: *NULLIND: *OMIT) CONST;
    i_time LIKE(tpl_sdk4i_vldrult_ds.max_time) OPTIONS(*NOPASS: *NULLIND: *OMIT) CONST;
    i_ts LIKE(tpl_sdk4i_vldrult_ds.max_ts) OPTIONS(*NOPASS: *NULLIND: *OMIT) CONST;
    i_lib LIKE(tpl_sdk4i_vldrult_ds.lib) OPTIONS(*NOPASS: *NULLIND: *OMIT) CONST;
    i_log_user_info_ds LIKEDS(tpl_sdk4i_log_user_info_ds) OPTIONS(*NOPASS: *NULLIND: *OMIT) CONST;
  END-PI;
  ...
  DCL-S fcol LIKE(tpl_sdk4i_vldrult_ds.fcol) NULLIND;
  DCL-S fhost LIKE(tpl_sdk4i_vldrult_ds.fhost) NULLIND;
  DCL-S flib LIKE(tpl_sdk4i_vldrult_ds.flib) NULLIND;
  DCL-S ftbl LIKE(tpl_sdk4i_vldrult_ds.ftbl) NULLIND;
  DCL-S full_rgx LIKE(tpl_sdk4i_vldrult_ds.rgx) NULLIND;
  DCL-S is_nullable CHAR(1) INZ('N');
  DCL-S max_date LIKE(tpl_sdk4i_vldrult_ds.max_date) NULLIND INZ(*HIVAL);
  DCL-S max_len LIKE(tpl_sdk4i_vldrult_ds.max_len) NULLIND INZ(*HIVAL);
  DCL-S max_num LIKE(tpl_sdk4i_vldrult_ds.max_num) NULLIND INZ(*HIVAL);
  DCL-S max_time LIKE(tpl_sdk4i_vldrult_ds.max_time) NULLIND INZ(*HIVAL);
  DCL-S max_ts LIKE(tpl_sdk4i_vldrult_ds.max_ts) NULLIND INZ(*HIVAL);
  DCL-S min_date LIKE(tpl_sdk4i_vldrult_ds.min_date) NULLIND INZ(*LOVAL);
  DCL-S min_len LIKE(tpl_sdk4i_vldrult_ds.min_len) NULLIND INZ(*LOVAL);
  DCL-S min_num LIKE(tpl_sdk4i_vldrult_ds.min_num) NULLIND INZ(*LOVAL);
  DCL-S min_time LIKE(tpl_sdk4i_vldrult_ds.min_time) NULLIND INZ(*LOVAL);
  DCL-S min_ts LIKE(tpl_sdk4i_vldrult_ds.min_ts) NULLIND INZ(*LOVAL);
  DCL-S vldmsgt_id LIKE(tpl_sdk4i_vldrult_ds.vldmsgt_id) NULLIND;
  DCL-S rgx LIKE(tpl_sdk4i_vldrult_ds.rgx) NULLIND;
  DCL-S rgx_id LIKE(tpl_sdk4i_vldrult_ds.rgx_id) NULLIND;
  DCL-S temp_date LIKE(i_date) NULLIND;
  DCL-S temp_lib LIKE(i_lib) NULLIND;
  DCL-S temp_num LIKE(i_num) NULLIND INZ(*LOVAL);
  DCL-S temp_rgx LIKE(tpl_sdk4i_vldrult_ds.rgx) NULLIND;
  ...
  EXEC SQL
    SELECT rgx, rgx_id, min_len, max_len, min_num, max_num, min_date, max_date, min_time, max_time,
      min_ts, max_ts, fhost, flib, ftbl, fcol, vldmsgt_id
    INTO :rgx :rgx_null, :rgx_id :rgx_id_null, :min_len :min_len_null, :max_len :max_len_null,
      ...
      :fhost :fhost_null, :flib :flib_null, :ftbl :ftbl_null, :fcol :fcol_null, :vldmsgt_id :vldmsgt_id_null
    FROM vldrult
  ...
  IF (ERR_IsSQLError(s_diagnostics_ds: *OMIT: *OMIT: log_user_info_ds));
    ...
    log_msg = 'Failed to retrieve validation rules from vldrult ('+ temp_lib +'/'+ i_tbl +'/'+ i_col +'). ' + s_diagnostics_ds.err_msg;
    LOG_LogMsg(psds_ds: log_proc: log_msg: log_cause_info_ds: log_event_info_ds: log_user_info_ds);
    log_is_successful = *OFF;
    RETURN log_is_successful;
  ENDIF;

  // If there is a vldmsgt_id associated with this validation rule (there should be!), return it to the
  // caller. If the one we retrieved is blank or NULL, return a generic ID.
  IF (vldmsgt_id_null = C_SDK4I_NULL);
    o_vldmsgt_id = 'GENERIC_FAILURE';
  ELSE;
  ...
  IF (%PARMS >= %PARMNUM(i_str) AND %ADDR(i_str) <> *NULL AND (rgx_null = C_SDK4I_NOT_NULL OR rgx_id_null = C_SDK4I_NOT_NULL));
    ...
    IF (rgx_id_null = C_SDK4I_NOT_NULL);
      ...
      ENDIF;
      full_rgx += %CHAR(']{': *UTF8) +
        %TRIM(%EDITC(min_len: 'P')) + ',' +
        %TRIM(%EDITC(max_len: 'P')) +
        %CHAR('}$': *UTF8);
      rgx = full_rgx;
...
DCL-PROC VLD_IsValidDate EXPORT;
  DCL-PI VLD_IsValidDate IND;
    i_date LIKE(tpl_sdk4i_vldrult_ds.max_date) OPTIONS(*NULLIND) CONST;
    i_min_date LIKE(tpl_sdk4i_vldrult_ds.min_date) OPTIONS(*NULLIND) CONST;
    i_max_date LIKE(tpl_sdk4i_vldrult_ds.max_date) OPTIONS(*NULLIND) CONST;
    i_log_user_info_ds LIKEDS(tpl_sdk4i_log_user_info_ds) OPTIONS(*NOPASS: *NULLIND: *OMIT) CONST;
  END-PI;
...
//
//   Note that this procedure has nothing to do with foreign key relationships defined in Db2 for i.
// This enforces relationships at the application level since many of our Physical Files do not have
...
DCL-PROC VLD_IsValidFK EXPORT;
  DCL-PI VLD_IsValidFK IND;
    i_ftbl LIKE(tpl_sdk4i_vldrult_ds.ftbl) OPTIONS(*TRIM) CONST;
    i_fcol LIKE(tpl_sdk4i_vldrult_ds.fcol) OPTIONS(*TRIM) CONST;
    i_str LIKE(tpl_sdk4i_sql_statement) OPTIONS(*NOPASS: *NULLIND: *OMIT) CONST;
    i_num LIKE(tpl_sdk4i_vldrult_ds.max_num) OPTIONS(*NOPASS: *NULLIND: *OMIT) CONST;
    i_date LIKE(tpl_sdk4i_vldrult_ds.max_date) OPTIONS(*NOPASS: *NULLIND: *OMIT) CONST;
    i_time LIKE(tpl_sdk4i_vldrult_ds.max_time) OPTIONS(*NOPASS: *NULLIND: *OMIT) CONST;
    i_ts LIKE(tpl_sdk4i_vldrult_ds.max_ts) OPTIONS(*NOPASS: *NULLIND: *OMIT) CONST;
    i_fhost LIKE(tpl_sdk4i_vldrult_ds.fhost) OPTIONS(*NOPASS: *OMIT: *TRIM) CONST;
    i_flib LIKE(tpl_sdk4i_vldrult_ds.flib) OPTIONS(*NOPASS: *OMIT: *TRIM) CONST;
    i_log_user_info_ds LIKEDS(tpl_sdk4i_log_user_info_ds) OPTIONS(*NOPASS: *NULLIND: *OMIT) CONST;
  END-PI;
...
DCL-PROC VLD_IsValidNumber EXPORT;
  DCL-PI VLD_IsValidNumber IND;
    i_num LIKE(tpl_sdk4i_vldrult_ds.max_num) OPTIONS(*NULLIND) CONST;
    i_min_num LIKE(tpl_sdk4i_vldrult_ds.min_num) OPTIONS(*NULLIND) CONST;
    i_max_num LIKE(tpl_sdk4i_vldrult_ds.max_num) OPTIONS(*NULLIND) CONST;
    i_log_user_info_ds LIKEDS(tpl_sdk4i_log_user_info_ds) OPTIONS(*NOPASS: *NULLIND: *OMIT) CONST;
  END-PI;
...
DCL-PROC VLD_IsValidTime EXPORT;
  DCL-PI VLD_IsValidTime IND;
    i_time LIKE(tpl_sdk4i_vldrult_ds.max_time) OPTIONS(*NULLIND) CONST;
    i_min_time LIKE(tpl_sdk4i_vldrult_ds.min_time) OPTIONS(*NULLIND) CONST;
    i_max_time LIKE(tpl_sdk4i_vldrult_ds.max_time) OPTIONS(*NULLIND) CONST;
    i_log_user_info_ds LIKEDS(tpl_sdk4i_log_user_info_ds) OPTIONS(*NOPASS: *NULLIND: *OMIT) CONST;
  END-PI;
...
DCL-PROC VLD_IsValidTimestamp EXPORT;
  DCL-PI VLD_IsValidTimestamp IND;
    i_ts LIKE(tpl_sdk4i_vldrult_ds.max_ts) OPTIONS(*NULLIND) CONST;
    i_min_ts LIKE(tpl_sdk4i_vldrult_ds.min_ts) OPTIONS(*NULLIND) CONST;
    i_max_ts LIKE(tpl_sdk4i_vldrult_ds.max_ts) OPTIONS(*NULLIND) CONST;
    i_log_user_info_ds LIKEDS(tpl_sdk4i_log_user_info_ds) OPTIONS(*NOPASS: *NULLIND: *OMIT) CONST;
  END-PI;
