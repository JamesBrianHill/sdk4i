...
//
//   Note that only the LOG_LogMsg and LOG_LogUse procedures are exposed to callers, the other
// procedures are private.
//
...
//
// Note: the i_psds_ds parameter cannot be *EXACT because of the nature of the data structure.
//
// Parameters:
...
DCL-PROC LOG_LogMsg EXPORT;
  ...
  DCL-S log_end_ts LIKE(tpl_sdk4i_logmett_ds.end_ts);
  DCL-S new_id LIKE(tpl_sdk4i_logmsgt_ds.id);
  DCL-S temp_new_id CHAR(15) CCSID(*UTF8);

  // Variables associated with LOGMSGT.
  DCL-S usrprf_cur LIKE(tpl_sdk4i_logmsgt_ds.usrprf_cur);
  DCL-S usrprf_ses LIKE(tpl_sdk4i_logmsgt_ds.usrprf_ses);
  ...
  DCL-S sys LIKE(tpl_sdk4i_logmsgt_ds.sys);
  DCL-S lib LIKE(tpl_sdk4i_logmsgt_ds.lib);
  DCL-S pgm LIKE(tpl_sdk4i_logmsgt_ds.pgm);
  DCL-S mod LIKE(tpl_sdk4i_logmsgt_ds.mod);
  DCL-S job_number LIKE(tpl_sdk4i_logmsgt_ds.job_number);
  DCL-S job_user LIKE(tpl_sdk4i_logmsgt_ds.job_user);
  DCL-S job_name LIKE(tpl_sdk4i_logmsgt_ds.job_name);
  ...
  DCL-S errline LIKE(tpl_sdk4i_logmsgt_ds.errline);
  DCL-S errrout LIKE(tpl_sdk4i_logmsgt_ds.errrout);
  DCL-S errdata LIKE(tpl_sdk4i_logmsgt_ds.errdata);

  DCL-S errcode_null LIKE(tpl_sdk4i_nil_null_int) INZ(C_SDK4I_NULL); // Assume NULL
  DCL-S errline_null LIKE(tpl_sdk4i_nil_null_int) INZ(C_SDK4I_NULL); // Assume NULL
  DCL-S errrout_null LIKE(tpl_sdk4i_nil_null_int) INZ(C_SDK4I_NULL); // Assume NULL
  DCL-S errdata_null LIKE(tpl_sdk4i_nil_null_int) INZ(C_SDK4I_NULL); // Assume NULL
  DCL-S sstate_null LIKE(tpl_sdk4i_nil_null_int) INZ(C_SDK4I_NULL); // Assume NULL
  DCL-S sstmt_null LIKE(tpl_sdk4i_nil_null_int) INZ(C_SDK4I_NULL); // Assume NULL
  DCL-S user_id_null LIKE(tpl_sdk4i_nil_null_int) INZ(C_SDK4I_NULL); // Assume NULL
  DCL-S username_null LIKE(tpl_sdk4i_nil_null_int) INZ(C_SDK4I_NULL); // Assume NULL
  ...
  job_name = %TRIM(i_psds_ds.job_name);
  job_number = i_psds_ds.job_number;
  job_user = %TRIM(i_psds_ds.username);
  lib = %TRIM(i_psds_ds.lib);
  mod = %TRIM(i_psds_ds.mod_prc);
  pgm = %TRIM(i_psds_ds.pgm_prc);
  sys = %TRIM(i_psds_ds.sys);
  usrprf_cur = %TRIM(i_psds_ds.cur_usr);
  usrprf_ses = %TRIM(i_psds_ds.username);
  ...
  IF (i_psds_ds.exc_type <> *BLANKS AND i_psds_ds.exc_num <> *BLANKS);
    ...
    errrout = %TRIM(i_psds_ds.routine);
    errrout_null = C_SDK4I_NOT_NULL;
    errdata = %TRIM(i_psds_ds.exc_data);
  ...
  // Extract data from i_log_user_info_ds if provided.
  IF (%PARMS >= %PARMNUM(i_log_user_info_ds) AND %ADDR(i_log_user_info_ds) <> *NULL);
    IF (i_log_user_info_ds.user_id > 0);
      user_id = i_log_user_info_ds.user_id;
      user_id_null = C_SDK4I_NOT_NULL;
    ENDIF;
    IF (i_log_user_info_ds.username <> *BLANKS);
      username = i_log_user_info_ds.username;
  ...
  IF (do_logcsit = 'Y');
    SaveCallStackInfo(new_id: i_log_user_info_ds);
  ENDIF;

  // Log extended information if configured to do so.
  IF (do_logextt = 'Y');
    SaveExtendedInfo(new_id: i_log_user_info_ds);
  ENDIF;
  ...
  IF (do_logwblt = 'Y' AND %PARMS >= %PARMNUM(i_logwblt_ds) AND %ADDR(i_logwblt_ds) <> *NULL);
    SaveLocalWebServiceInfo(new_id: i_logwblt_ds: i_log_user_info_ds);
  ENDIF;
  ...
  IF (do_logwbrt = 'Y' AND %PARMS >= %PARMNUM(i_logwbrt_ds) AND %ADDR(i_logwbrt_ds) <> *NULL);
    SaveRemoteWebServiceInfo(new_id: i_logwbrt_ds: i_log_user_info_ds);
  ENDIF;

  // If an event handler has been defined for the log level of the message, execute that event handler.
  CLEAR cmd;
  SELECT;
    WHEN (msg_lvl = C_SDK4I_LL_EMG AND emgcmd <> *BLANKS);
      cmd = emgcmd;
    WHEN (msg_lvl = C_SDK4I_LL_ALT AND altcmd <> *BLANKS);
      cmd = altcmd;
    WHEN (msg_lvl = C_SDK4I_LL_CRT AND crtcmd <> *BLANKS);
      cmd = crtcmd;
    WHEN (msg_lvl = C_SDK4I_LL_ERR AND errcmd <> *BLANKS);
      cmd = errcmd;
    WHEN (msg_lvl = C_SDK4I_LL_WRN AND wrncmd <> *BLANKS);
      cmd = wrncmd;
    WHEN (msg_lvl = C_SDK4I_LL_INF AND infcmd <> *BLANKS);
      cmd = infcmd;
    WHEN (msg_lvl = C_SDK4I_LL_DBG AND dbgcmd <> *BLANKS);
      cmd = dbgcmd;
    OTHER;
      // Don't do anything.
  ENDSL;
  ...
  IF (cmd <> *BLANKS);
    // If the string &SDK4I_ID appears anywhere in the command, replace it with the value in new_id.
  ...
  ON-EXIT log_is_abend;
    ...
    LOG_LogUse(psds_ds: log_proc: log_beg_ts: log_is_successful: log_is_abend: log_user_info_ds: log_end_ts);
END-PROC LOG_LogMsg;
...
DCL-PROC LOG_LogUse EXPORT;
  ...
  DCL-S do_logmett LIKE(tpl_sdk4i_logcfgt_ds.logmett) INZ('N');
  DCL-S do_loguset LIKE(tpl_sdk4i_logcfgt_ds.loguset) INZ('N');
  ...
  DCL-S local_is_abend LIKE(i_abend) NULLIND INZ(*OFF);
  DCL-S local_is_successful LIKE(i_abend) NULLIND INZ(*ON);
  ...
  IF (do_logmett = 'Y');
    ...
    IF (%PARMS >= %PARMNUM(i_log_user_info_ds) AND %ADDR(i_log_user_info_ds) <> *NULL);
      IF (i_log_user_info_ds.user_id > 0);
        temp_user_info_ds.user_id = i_log_user_info_ds.user_id;
        %NULLIND(temp_user_info_ds.user_id) = *OFF;
      ELSE;
        %NULLIND(temp_user_info_ds.user_id) = *ON;
      ENDIF;
      IF (i_log_user_info_ds.username <> *BLANKS);
        temp_user_info_ds.username = i_log_user_info_ds.username;
  ...
  ON-EXIT local_is_abend;
    ...
    IF (do_logmett = 'Y');
      log_end_ts = %TIMESTAMP(*SYS);
      // NOTE - we pass psds_ds as the first parameter, NOT i_psds_ds.
...
//
// References:
// @link https://www.ibm.com/docs/en/i/7.6.0?topic=services-stack-info-table-function
///
// -------------------------------------------------------------------------------------------------
DCL-PROC SaveCallStackInfo;
  DCL-PI SaveCallStackInfo;
    i_id LIKE(tpl_sdk4i_logmsgt_ds.id) OPTIONS(*EXACT) CONST;
    i_log_user_info_ds LIKEDS(tpl_sdk4i_log_user_info_ds) OPTIONS(*NOPASS: *NULLIND: *OMIT) CONST;
  END-PI;
...
DCL-PROC SaveLocalWebServiceInfo;
  ...
  DCL-S libl_null LIKE(tpl_sdk4i_nil_null_int) INZ(C_SDK4I_NULL); // Assume NULL
  DCL-S protocol_null LIKE(tpl_sdk4i_nil_null_int) INZ(C_SDK4I_NULL); // Assume NULL
  DCL-S query_null LIKE(tpl_sdk4i_nil_null_int) INZ(C_SDK4I_NULL); // Assume NULL
  DCL-S req_method_null LIKE(tpl_sdk4i_nil_null_int) INZ(C_SDK4I_NULL); // Assume NULL
  DCL-S rmt_ipv4_null LIKE(tpl_sdk4i_nil_null_int) INZ(C_SDK4I_NULL); // Assume NULL
  DCL-S rmt_port_null LIKE(tpl_sdk4i_nil_null_int) INZ(C_SDK4I_NULL); // Assume NULL
  DCL-S rsp_code_null LIKE(tpl_sdk4i_nil_null_int) INZ(C_SDK4I_NULL); // Assume NULL
  DCL-S script_null LIKE(tpl_sdk4i_nil_null_int) INZ(C_SDK4I_NULL); // Assume NULL
  DCL-S srv_ipv4_null LIKE(tpl_sdk4i_nil_null_int) INZ(C_SDK4I_NULL); // Assume NULL
  DCL-S srv_port_null LIKE(tpl_sdk4i_nil_null_int) INZ(C_SDK4I_NULL); // Assume NULL
  DCL-S uri_null LIKE(tpl_sdk4i_nil_null_int) INZ(C_SDK4I_NULL); // Assume NULL
...
// took to execute, etc.
//
// NOTE: We cannot use *EXACT for i_psds_ds because in the LOG_LogUse procedure we send an actual
// psds, not a tpl_sdk4i_psds_ds.
//
// Parameters:
// @param REQUIRED. A populated tpl_sdk4i_psds_ds data structure.
// @param REQUIRED. The name of the calling procedure.
...
// @param OPTIONAL. A timestamp for when this process ended.
///
// -------------------------------------------------------------------------------------------------
DCL-PROC SaveMetrics;
  DCL-PI SaveMetrics;
    ...
    i_beg_ts LIKE(tpl_sdk4i_logmett_ds.beg_ts) OPTIONS(*EXACT) CONST;
    i_successful LIKE(tpl_sdk4i_is_successful) OPTIONS(*NOPASS: *NULLIND: *OMIT) CONST;
    i_abend LIKE(tpl_sdk4i_is_abend) OPTIONS(*NOPASS: *NULLIND: *OMIT) CONST;
    i_log_user_info_ds LIKEDS(tpl_sdk4i_log_user_info_ds) OPTIONS(*NOPASS: *NULLIND: *OMIT) CONST;
    i_end_ts LIKE(tpl_sdk4i_logmett_ds.end_ts) OPTIONS(*NOPASS: *NULLIND: *OMIT) CONST;
  END-PI;
  ...
  DCL-S mod LIKE(i_psds_ds.mod_prc);
  DCL-S pgm LIKE(i_psds_ds.pgm_prc);
  DCL-S success LIKE(tpl_sdk4i_logmett_ds.success) INZ(*ON);
  DCL-S sys LIKE(i_psds_ds.sys);
  ...
  DCL-S abend_null LIKE(tpl_sdk4i_nil_null_int) INZ(C_SDK4I_NULL); // Assume NULL
  DCL-S success_null LIKE(tpl_sdk4i_nil_null_int) INZ(C_SDK4I_NULL); // Assume NULL
  DCL-S user_id_null LIKE(tpl_sdk4i_nil_null_int) INZ(C_SDK4I_NULL); // Assume NULL
  DCL-S username_null LIKE(tpl_sdk4i_nil_null_int) INZ(C_SDK4I_NULL); // Assume NULL
...
DCL-PROC SaveRemoteWebServiceInfo;
  ...
  // -----------------------------------------------
  // Define local variables.
  // NOTE: columns req_body and rsp_body are defined
  // as CLOB in the table. As of IBM i 7.5 we cannot
  // use LIKE() with columns defined that way so we
  // have to use SQLTYPE(CLOB: x) below.
  // -----------------------------------------------
  DCL-S log_end_ts LIKE(tpl_sdk4i_logmett_ds.end_ts);
  DCL-S req_url LIKE(tpl_sdk4i_logwbrt_ds.req_url);
  DCL-S req_method LIKE(tpl_sdk4i_logwbrt_ds.req_method);
  DCL-S req_opt LIKE(tpl_sdk4i_logwbrt_ds.req_opt);
  DCL-S req_body SQLTYPE(CLOB: 2097152); // Keep this size in sync with LOGWBRT.req_body!
  DCL-S rsp_code LIKE(tpl_sdk4i_logwbrt_ds.rsp_code);
  DCL-S rsp_head LIKE(tpl_sdk4i_logwbrt_ds.rsp_head);
  DCL-S rsp_body SQLTYPE(CLOB: 4194304); // Keep this size in sync with LOGWBRT.req_body!

  DCL-S req_url_null LIKE(tpl_sdk4i_nil_null_int) INZ(C_SDK4I_NULL); // Assume NULL
  DCL-S req_method_null LIKE(tpl_sdk4i_nil_null_int) INZ(C_SDK4I_NULL); // Assume NULL
  DCL-S req_opt_null LIKE(tpl_sdk4i_nil_null_int) INZ(C_SDK4I_NULL); // Assume NULL
  DCL-S req_body_null LIKE(tpl_sdk4i_nil_null_int) INZ(C_SDK4I_NULL); // Assume NULL
  DCL-S rsp_code_null LIKE(tpl_sdk4i_nil_null_int) INZ(C_SDK4I_NULL); // Assume NULL
  DCL-S rsp_head_null LIKE(tpl_sdk4i_nil_null_int) INZ(C_SDK4I_NULL); // Assume NULL
  DCL-S rsp_body_null LIKE(tpl_sdk4i_nil_null_int) INZ(C_SDK4I_NULL); // Assume NULL
  ...
  // Main logic.
  // -----------------------------------------------
  req_url = i_logwbrt_ds.req_url;
  req_url_null = NIL_IndToInt(%NULLIND(i_logwbrt_ds.req_url));
  req_method = i_logwbrt_ds.req_method;
  req_method_null = NIL_IndToInt(%NULLIND(i_logwbrt_ds.req_method));
  req_opt = i_logwbrt_ds.req_opt;
  req_opt_null = NIL_IndToInt(%NULLIND(i_logwbrt_ds.req_opt));

  // We have to handle req_body differently since it's a CLOB.
  req_body_data = i_logwbrt_ds.req_body;
  req_body_len = %LEN(i_logwbrt_ds.req_body);
  ...
  rsp_head = i_logwbrt_ds.rsp_head;
  rsp_head_null = NIL_IndToInt(%NULLIND(i_logwbrt_ds.rsp_head));

  // We have to handle rsp_body differently since it's a CLOB.
  rsp_body_data = i_logwbrt_ds.rsp_body;
  rsp_body_len = %LEN(i_logwbrt_ds.rsp_body);
  rsp_body_null = NIL_IndToInt(%NULLIND(i_logwbrt_ds.rsp_body));

  EXEC SQL
    INSERT INTO logwbrt (logmsgt_id, req_url, req_method, req_opt, req_body, rsp_code, rsp_head, rsp_body)
    VALUES(
      :i_id,
      :req_url :req_url_null,
      :req_method :req_method_null,
      :req_opt :req_opt_null,
      CAST(:req_body :req_body_null AS CLOB(2097152)),
      :rsp_code :rsp_code_null,
      :rsp_head :rsp_head_null,
      CAST(:rsp_body :rsp_body_null AS CLOB(4194304))
      )
    WITH NC;
  ON-EXIT log_is_abend;
    log_end_ts = %TIMESTAMP(*SYS);
...
//
// Overview:
//   We may want to log the fact a program, procedure, etc. has been used. The LOGCFGT table allows
// us to indicate if we should track usage and how fine-grained that tracking should be (yearly,
...
//
//   If you want to use the WEEK_ISO scalar function instead, be aware that it returns a value
// between 1 and 53 so it's possible for the function to return a week that is in the previous or
// next year - which means you need to decide whether or not to change the year and month values to
// match the previous/next year when you encounter an edge case.
//
// Parameters:
// @param REQUIRED. A populated tpl_sdk4i_psds_ds data structure.
// @param REQUIRED. The name of the calling procedure.
...
DCL-PROC SaveUseInfo;
  ...
  DCL-S cur_month LIKE(tpl_sdk4i_loguset_ds.mnth) INZ(C_SDK4I_NULL);
  ...
  EXEC SQL
    ...
    -- @link https://www.ibm.com/docs/en/i/7.6.0?topic=statements-merge
