...
DCL-PROC COM_SendBreakMessage EXPORT;
  ...
  // Main logic.
  // -----------------------------------------------
  temp_msg = i_msg; // The API is expecting a CHAR, not a VARCHAR, so we use a temp variable.

  MONITOR;
    API_SendMessage('*INFO': '*BREAK': temp_msg: %LEN(%TRIM(temp_msg)): i_usrprf: 1: sent: function_requested:
...
// @param OPTIONAL. The message type (0=text/plain, 1=text/html, 2=text/xml). Defaults to 0.
...
DCL-PROC COM_SendEmail EXPORT;
  DCL-PI COM_SendEmail IND;
    ...
    i_msg POINTER VALUE;
    i_msg_content_type LIKE(tpl_sdk4i_unsigned_binary4) OPTIONS(*NOPASS: *OMIT) CONST;
  ...
  ON-ERROR 00425;
    ...
    log_msg = 'Failed to allocate memory ('+ %TRIM(%EDITC(recipient_data_size: 'P')) +') for email recipients: 00425 - requested allocation size is out of range.';
    LOG_LogMsg(psds_ds: log_proc: log_msg: log_cause_info_ds: log_event_info_ds: log_user_info_ds);
    RESET log_event_info_ds;
    log_is_successful = *OFF;
    RETURN log_is_successful;
  ON-ERROR 00426;
    log_event_info_ds.fac_id = 2; // Mail server
    log_msg = 'Failed to allocate memory ('+ %TRIM(%EDITC(recipient_data_size: 'P')) +') for email recipients: 00426 - error occurred during memory allocation.';
    LOG_LogMsg(psds_ds: log_proc: log_msg: log_cause_info_ds: log_event_info_ds: log_user_info_ds);
    RESET log_event_info_ds;
    log_is_successful = *OFF;
    RETURN log_is_successful;
  ON-ERROR;
    log_event_info_ds.fac_id = 2; // Mail server
    log_msg = 'Failed to allocate memory ('+ %TRIM(%EDITC(recipient_data_size: 'P')) +') for email recipients: Unknown error.';
    LOG_LogMsg(psds_ds: log_proc: log_msg: log_cause_info_ds: log_event_info_ds: log_user_info_ds);
    RESET log_event_info_ds;
    log_is_successful = *OFF;
    RETURN log_is_successful;
  ...
  FOR x = 1 TO i_recipient_count; // Loop through all recipients.
    ...
    // 16-byte boundary.
    API_CopyWithPointer(p_temp: %ADDR(recipient_ds): %LEN(recipient_ds));

    //   We use the MEMCPY function because the data is almost certainly NOT aligned on a 16-byte
    // boundary.
    API_CopyMemory(p_recipient + offset: %ADDR(i_recipient_array(x).address: *DATA): %LEN(i_recipient_array(x).address));
  ...
  ON-ERROR 00425;
    log_event_info_ds.fac_id = 2; // Mail server
    log_msg = 'Failed to allocate memory ('+ %TRIM(%EDITC(note_data_size: 'P')) +') for email note: 00425 - requested allocation size is out of range.';
    LOG_LogMsg(psds_ds: log_proc: log_msg: log_cause_info_ds: log_event_info_ds: log_user_info_ds);
    RESET log_event_info_ds;
    log_is_successful = *OFF;
    RETURN log_is_successful;
  ON-ERROR 00426;
    log_event_info_ds.fac_id = 2; // Mail server
    log_msg = 'Failed to allocate memory ('+ %TRIM(%EDITC(note_data_size: 'P')) +') for email note: 00426 - error occurred during memory allocation.';
    LOG_LogMsg(psds_ds: log_proc: log_msg: log_cause_info_ds: log_event_info_ds: log_user_info_ds);
    RESET log_event_info_ds;
    log_is_successful = *OFF;
    RETURN log_is_successful;
  ON-ERROR;
    log_event_info_ds.fac_id = 2; // Mail server
    log_msg = 'Failed to allocate memory ('+ %TRIM(%EDITC(note_data_size: 'P')) +') for email note: Unknown error.';
    LOG_LogMsg(psds_ds: log_proc: log_msg: log_cause_info_ds: log_event_info_ds: log_user_info_ds);
    RESET log_event_info_ds;
    log_is_successful = *OFF;
    RETURN log_is_successful;
  ...
  IF (%PARMS >= %PARMNUM(i_msg_content_type) AND %ADDR(i_msg_content_type) <> *NULL);
    note_ds.note_content_type = i_msg_content_type;
  ENDIF;
  ...
  IF (attachment_count > 0);
    ...
    ON-ERROR 00425;
      log_event_info_ds.fac_id = 2; // Mail server
      log_msg = 'Failed to allocate memory ('+ %TRIM(%EDITC(attachment_data_size: 'P')) +') for email recipients: 00425 - requested allocation size is out of range.';
      LOG_LogMsg(psds_ds: log_proc: log_msg: log_cause_info_ds: log_event_info_ds: log_user_info_ds);
      RESET log_event_info_ds;
      log_is_successful = *OFF;
      RETURN log_is_successful;
    ON-ERROR 00426;
      log_event_info_ds.fac_id = 2; // Mail server
      log_msg = 'Failed to allocate memory ('+ %TRIM(%EDITC(attachment_data_size: 'P')) +') for email recipients: 00426 - error occurred during memory allocation.';
      LOG_LogMsg(psds_ds: log_proc: log_msg: log_cause_info_ds: log_event_info_ds: log_user_info_ds);
      RESET log_event_info_ds;
      log_is_successful = *OFF;
      RETURN log_is_successful;
    ON-ERROR;
      log_event_info_ds.fac_id = 2; // Mail server
      log_msg = 'Failed to allocate memory ('+ %TRIM(%EDITC(attachment_data_size: 'P')) +') for email recipients: Unknown error.';
      LOG_LogMsg(psds_ds: log_proc: log_msg: log_cause_info_ds: log_event_info_ds: log_user_info_ds);
      RESET log_event_info_ds;
      log_is_successful = *OFF;
      RETURN log_is_successful;
    ...
    // stat() functions (and probably some other methods) but it is easier to call the
    ...
    FOR x = 1 TO attachment_count;
      ...
      API_CopyWithPointer(p_temp: %ADDR(attachment_ds): %LEN(attachment_ds));

      // Copy the full pathname of the attachment to our allocated memory.
      API_CopyMemory(p_attachment + offset: %ADDR(i_attachment_array(x): *DATA): %LEN(i_attachment_array(x)));
  ...
  // ===========================================================================
  API_CreateAndSendEmail(p_recipient: // Recipient data
  ...
  /ELSE
  log_is_successful = *OFF; // If we are on PUB400 this procedure is not available.
  /ENDIF
