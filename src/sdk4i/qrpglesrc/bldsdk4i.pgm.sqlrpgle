...
//
//   You only need to execute this program when you're installing SDK4i for the first time. Before
// you compile this program and run it, please check the following values to ensure they are what
// you want them to be:
//
// *) C_ADD_RESTRICT_ON_DROP - set this equal to 'Y' (the default) if you want to compile all the
//    SDK4i tables with the RESTRICT ON DROP clause. You can read more about this clause here:
//    https://www.ibm.com/docs/en/i/7.6.0?topic=object-restrict-drop
// *) C_LIBDTA - this is the library you created to hold all the SDK4i database objects. It defaults
//    to SDK4IDTA.
// *) C_LIBPGM - this is the library you created to hold all the SDK4i program objects that are not
...
DCL-PROC BLDSDK4I;
  ...
  // The order in which components are built is important because some of them depend on others.
  BuildComponent('NIL': 'SDK4i - NIL - NULL handling utilities': 'SDK4I': 'Y'); // Needs LOG
  BuildComponent('LOG': 'SDK4i - LOG - Logging utilities': 'SDK4I'); // Needs NIL
...
DCL-PROC BuildComponent;
  ...
  temp_module_path = C_SRC_ROOT + %LOWER(i_name) + '/qrpglesrc/' + %LOWER(i_name) + '.sqlrpgle';
  temp_srvpgm_path = C_SRC_ROOT + %LOWER(i_name) + '/qsrvsrc/' + %LOWER(i_name) + '.bnd';
...
// -------------------------------------------------------------------------------------------------
///
//   Build a service program.
//
//   We compile a source member into a module, then into a service program and add that service
// program to a binding directory if provided.
///
// -------------------------------------------------------------------------------------------------
DCL-PROC BuildSRVPGM;
  DCL-PI BuildSRVPGM;
    i_component VARCHAR(10) CONST;
    i_name VARCHAR(10) CONST;
    i_text VARCHAR(50) CONST;
    i_bnddir VARCHAR(10) CONST;
    i_allow_unresolved CHAR(1) OPTIONS(*NOPASS) CONST;
  ...
  temp_module_path = C_SRC_ROOT + %LOWER(i_component) + '/qrpglesrc/'+ %LOWER(i_name) + '.sqlrpgle';
  temp_srvpgm_path = C_SRC_ROOT + %LOWER(i_component) + '/qsrvsrc/'+ %LOWER(i_name) + '.bnd';
...
DCL-PROC LogMsg;
  ...
  IF (SQLSTATE <> C_SDK4I_SQLSTATE_OK);
    SND-MSG *INFO 'LogMsg: PREPARE failed: '+ s_stmt;
    RETURN;
  ENDIF;
  ...
  IF (SQLSTATE <> C_SDK4I_SQLSTATE_OK);
    SND-MSG *INFO 'LogMsg: EXECUTE failed: '+ s_stmt;
    RETURN;
  ENDIF;
...
DCL-PROC RunSQLStatement;
  ...
  IF (%ADDR(i_lib) <> *NULL AND i_lib <> *BLANKS);
    cmd = 'RUNSQLSTM DFTRDBCOL('+ %TRIM(i_lib) +') SRCSTMF('+ C_SDK4I_QUOTE + i_src + C_SDK4I_QUOTE +
      ') DATFMT(*ISO) DECMPT(*PERIOD) MARGINS(240) TIMFMT(*ISO) COMMIT(*NONE)';
  ELSE;
    cmd = 'RUNSQLSTM SRCSTMF('+ C_SDK4I_QUOTE + i_src + C_SDK4I_QUOTE +
      ') DATFMT(*ISO) DECMPT(*PERIOD) MARGINS(240) TIMFMT(*ISO) COMMIT(*NONE)';
  ENDIF;
  MONITOR;
    API_ExecuteCommand(cmd: %LEN(cmd));
...
//
//   We want to ensure our SDK4i libraries are in the library list. We handle the following specific
// exceptions:
