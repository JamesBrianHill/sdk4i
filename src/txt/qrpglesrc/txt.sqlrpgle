  ...
  IF (i_position = 'C');
    ...
    ON-ERROR;
      log_msg = 'Error executing %SUBST when centering a string. ' +
        'start = ' + %TRIM(%EDITC(start: 'P')) +
        'str_char_count = ' + %TRIM(%EDITC(str_char_count: 'P'));
      LOG_LogMsg(psds_ds: log_proc: log_msg: log_cause_info_ds: log_event_info_ds: log_user_info_ds);
      log_is_successful = *OFF;
    ENDMON;
    RETURN o_str;
...
DCL-PROC TXT_Q EXPORT;
  ...
  // @link https://www.ibm.com/docs/en/i/7.6.0?topic=functions-scanrpl-scan-replace-characters#bbscanrp
  o_str += %SCANRPL(
...
// @link https://www.ibm.com/docs/en/i/7.6.0?topic=functions-strtok-tokenize-string
//
